...
use conduit_core::{
    ...
    snapshot::{RuntimeSnapshot, SnapshotStore},
...
async fn get_trace_returns_events_after_traced_query() {
    ...
    let local_addr =
        conduit_api::serve_on_listener(addr, snapshots.clone(), effective, tracing.clone())
            .await
            .expect("start server");
...
async fn get_trace_unknown_id_not_found() {
    ...
    let addr: SocketAddr = "127.0.0.1:0".parse().expect("parse");
    let local_addr = conduit_api::serve_on_listener(addr, snapshots, effective, tracing.clone())
        .await
        .expect("start server");
