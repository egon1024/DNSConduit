...
For hook timing, pipeline placement, first-match evaluation, built-in actions, and YAML wiring, see [Rules and actions — Request and response hooks](/policy-routing/rules-and-actions.md#request-and-response-hooks). For method-level detail (`txn.set_pool`, `txn.drop_query`, …), see [Transaction API](/rhai/transaction-api.md).
...
| `txn.clear_tag`, `txn.has_tag`, `txn.set_pool`, `txn.set_tag` | Yes | Yes |
...
1. **[Request hook](#request-hook)** — classify the query (lookup table, qname pattern) and **`txn.set_tag`**
...
- [Retries and transactions](/policy-routing/retries-and-transactions.md) — [Pool selection lifecycle](/policy-routing/retries-and-transactions.md#pool-selection-lifecycle), [Source selection lifecycle](/policy-routing/retries-and-transactions.md#source-selection-lifecycle), `set_retry_pool`, `set_retry_source_*`, `retry`, attempt limits, pool exhaustion
