...
- **Memory vs LMDB (churn)** — Under matched high churn (ingress-8, fixed safe sync), LMDB costs about **99%** QPS versus memory (~3k vs ~268k; roughly **93.4×**). Hit rates stay similar; fill means are about **3.0 ms** vs **0.0005 ms**. → [Memory vs LMDB high-churn cache](/performance/studies/memory-vs-lmdb-cache-churn.md)
