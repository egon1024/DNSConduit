...
For LMDB write durability, start from the [**`lmdb.sync` decision tree**](/reference/config-schema/caches.md#lmdb-sync). It includes **`periodic`**, which bounds crash loss to the last successful forced sync; tune its Hot **`lmdb.sync_interval`** there.
