...
- **Ubuntu 22.04 / 24.04 (amd64)** target. Each release publishes stripped and unstripped tarballs, production and debug Debian packages, `SHA256SUMS`, an SPDX **SBOM**, and a container image on **GHCR** (also mirrored to **Docker Hub**; reference and digest recorded in `conduit-<version>.image-digest.txt`). See [Install and run](/getting-started/install-and-run.md).
